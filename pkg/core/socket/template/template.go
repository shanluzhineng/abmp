...
import (
	...
	sockaddr "github.com/shanluzhineng/abmp/pkg/core/socket"
...
func Parse(input string) (string, error) {
	...
	if err != nil {
		return "", multierror.Wrapf("unable to query interface addresses: {{err}}", err)
	}
...
func ParseIfAddrsTemplate(input string, ifAddrs sockaddr.IfAddrs, tmplIn *template.Template) (string, error) {
	...
	tmpl, err := tmplIn.Option("missingkey=error").
		...
		Parse(input)
	if err != nil {
		return "", multierror.Wrapf(fmt.Sprintf("unable to parse template %+q: {{err}}", input), err)
	}

	var outWriter bytes.Buffer
	err = tmpl.Execute(&outWriter, ifAddrs)
	if err != nil {
		return "", multierror.Wrapf(fmt.Sprintf("unable to execute sockaddr input %+q: {{err}}", input), err)
	}
